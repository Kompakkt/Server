...
export class WikibaseService {
  ...
  public async updateAnnotation(
    ...
    if (!id) {
      ...
      id = await this.wbEdit.entity
        ...
        .then(created => created.entity.id as `Q${number}`)
    ...
    const spark = {
      ...
      clear: true,
      labels: {
        en: annotation.label.en,
      },
      descriptions: {
        en: annotation.description.en,
      },
      claims: {
    ...
    } satisfies EditEntitySimplifiedModeParams;
  ...
  public async updateDigitalEntity(fullEntity: ServerDocument<WikibaseDigitalEntity>) {
    ...
    if (modelAgents['Data Creator'].length > 0) {
      ...
      (editParams.claims[predicate] as SimplifiedEditableClaim[]).push({
  ...
  public async removeWikibaseItem(id: string) {
    ...
    return this.wbEdit.entity.delete({ id: id });
  }
