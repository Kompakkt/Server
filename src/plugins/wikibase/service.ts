...
const resolveIdFromCreateError = async (
  ...
  if (label) {
    const search = await simpleItemSearch({ q: label, language: 'en', limit: 1 });
    if (search.status === 200) {
      const found = search.data.results[0]?.id;
      if (found) {
        log('Resolved existing Wikibase item id by label search', found);
        return found;
      }
    }
  }
...
const createItem = async (request: AddItemBody): Promise<string | undefined> => {
  try {
    ...
    return await resolveIdFromCreateError(res, request.item.labels?.['en']);
  } catch (error) {
    err(error);
    return undefined;
  }
...
const isQID = (str: string): str is `Q${number}` => {
  return /^Q\d+$/.test(str);
};
...
export class WikibaseService {
  ...
  public async updateAnnotation(
    ...
    if (!id) {
      const createRequest: AddItemBody = {
        item: {
          type: 'item',
          labels: annotation.label,
          descriptions: autogeneratedDescription,
        },
      };
      log('createRequest', createRequest);
      id = await createItem(createRequest);
    ...
    if (!isQID(id)) {
      throw new Error(`Generated id ${id} is not a valid Wikibase QID`);
    }

    const wikibaseDomain = WikibaseConfiguration?.Public ?? WikibaseConfiguration?.Domain;

    const wbClaims: SimplifiedClaims = {
      ...
      [WBAnnotationPredicates.relatedMedia]: annotation.media.map(getIdFromWikibaseItem),
    ...
    if (imageFilename) {
      wbClaims[WBPredicates.image] = imageFilename;
    }

    const statements: RestStatements = buildStatements(wbClaims);
    log('updateAnnotation statements', statements);
    const patchResponse = await patchItem(id, {
      patch: replaceItemPatch({
        statements,
        labels: { en: annotation.label.en },
        descriptions: { en: annotation.description.en },
      }),
    }).catch(error => {
      err(`Failed editing annotation: ${error}`);
      return undefined;
    });
    log('updateAnnotation result', patchResponse);

    if (targetEntity) {
      if (!isQID(targetEntity)) {
        throw new Error(`Target entity ${targetEntity} is not a valid Wikibase QID`);
      }
      const linkbackResponse = await addItemStatement(targetEntity, {
        statement: {
          property: { id: WBPredicates.hasAnnotation },
          value: { type: 'value', content: id },
        },
      }).catch(error => {
        log('Linkback best-effort add failed:', error);
        return undefined;
      });
      if (linkbackResponse && linkbackResponse.status >= 400) {
        log('Linkback add returned error status', linkbackResponse.status);
      } else {
        log('Linkback result:', linkbackResponse);
      }
    }
  ...
  public async updateDigitalEntity(fullEntity: ServerDocument<WikibaseDigitalEntity>) {
    ...
    if (imageFilename) {
      claims[WBPredicates.image] = imageFilename;
    }
    ...
    if (modelAgents['Data Creator'].length > 0) {
      ...
      const existing =
        (claims[predicate] as unknown as
          | Array<{ value: string; qualifiers: Record<string, string[]> }>
          | undefined) ?? [];
      claims[predicate] = [
        ...existing,
        {
          value: WBClasses.rawDataCreation,
          qualifiers: { [WBPredicates.carriedOutBy]: modelAgents['Data Creator'] },
        },
      ];
    }
    ...
    if (modelAgents['Creator'].length > 0) {
      ...
      claims[predicate] = [
        ...existing,
        {
          ...
          qualifiers: { [WBPredicates.carriedOutBy]: modelAgents['Creator'] },
        },
      ];
    }
    ...
    if (modelAgents['Editor'].length > 0) {
      ...
      claims[predicate] = [
        ...existing,
        {
          ...
          qualifiers: { [WBPredicates.carriedOutBy]: modelAgents['Editor'] },
        },
      ];
    }
    ...
    const statements = buildStatements(claims);
    log('updateDigitalEntity statements', statements);
    const editResponse = await patchItem(id, {
      patch: replaceItemPatch({
        statements,
        labels: { en: entity.label.en },
        descriptions: { en: entity.description.en },
      }),
    }).catch(error => {
      err(`Failed editing digital entity: ${error}`);
      return undefined;
    });
    log('updateDigitalEntity edit result', editResponse);
    ...
    return { itemId: id, claims, hierarchies: processHierarchies.flat() };
  }
  ...
  public async removeWikibaseItem(id: string) {
    if (!isQID(id)) {
      throw new Error(`Provided id ${id} is not a valid Wikibase QID`);
    }
    return this.wbConnect.removeItem(id);
  }
  ...
  public async fetchWikibaseMetadata(
    ...
    const hierarchies = await (async () => {
      if (!processedMetadata.physicalObjs || processedMetadata.physicalObjs.length <= 0) return [];
      const physicalObjIds = processedMetadata.physicalObjs
        .filter(item => typeof item === 'object' && 'value' in item)
        .map(item => item.value);
      const hierarchies = await Promise.all(physicalObjIds.map(id => this.fetchHierarchy(id)));
      return hierarchies.flat();
    })();
  ...
  public getAgentsFromMetadata(metadata: ProcessedMetadata): IMediaAgent[] {
    ...
    const addAgents = (roleTitle: IMediaAgent['roleTitle'], roleKey: string) => {
