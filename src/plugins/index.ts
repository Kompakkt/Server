...
export const PluginController = new (class {
  ...
  async loadPlugin(plugin: Plugin, pluginArgs?: unknown) {
    ...
    const routes = plugin.routers.flatMap(getRoutes).filter(route => !configServerRoutes.includes(route));
    log(`Registered plugin ${plugin.constructor.name} with ${routes.length} routes\n${routes.join('\n')}`);
    return true;
  }
})();
